...
router.get("/:user_id", async(req, res) => {
    ...
    try {
        await connection.beginTransaction();
        const [result] = await connection.query(`select o.overtime_index, u.name, date_format(r.work_start, '%Y-%m-%d %H:%i:%s') as work_start, date_format(r.work_end, '%Y-%m-%d %H:%i:%s') as work_end, wt.work_type_name, r.recruit_state, '초과 근무 요청' as type from overtime o
